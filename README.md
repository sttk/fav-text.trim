...
Remove leading and trailing white spaces or specified characters from a source string.
...
For Node.js, when installing `@fav/text.trim` from npm:

```js
var trim = require('@fav/text.trim');
trim('  abc   '); // => 'abc'
trim('_-abc--_-', 8, '_-'); // => 'abc'
```
